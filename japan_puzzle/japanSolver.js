...
var CellType;
(function (CellType) {
    CellType[CellType["filled"] = 1] = "filled";
    CellType[CellType["closed"] = 0] = "closed";
    CellType[CellType["empty"] = -100] = "empty";
})(CellType || (CellType = {}));
function getPuzzleSum(rows) {
    let sum = 0;
    for (let row of rows) {
        sum += row.reduce((a, b) => {
            a = a === -1 ? 0 : a;
...
function createCanvas(rows, cols, picture) {
...
function getLonger(collection) {
    return Math.max(...collection.map(arr => arr.length));
}
function firstIteration(rows, cols) {
    const rowValues = [];
    const colValues = [];
    for (let row of rows) {
...
function combineResults(rows, cols) {
...
function makePicture(rows, cols) {
    const newPicture = [];
    for (let row of rows) {
        newPicture.push(row);
    }
...
function convertToArray(values, perm, size) {
...
function checkPerm(truePerm, perm) {
...
//# sourceMappingURL=japanSolver.js.map