...
function checkChainRule(grid, calcEmpty = false) {
    let start = { x: -1, y: -1 };
    ...
    function traverse(neighbors) {
        ...
        for (let n of neighbors) {
            if (n !== null) {
                testGrid[n.x][n.y] = -1;
                const newNeighbors = helpers_1.get4Neighbors(testGrid, n).filter(i => i && testGrid[i.x][i.y] === 0);
...
function makeNeighborsPermutations() {
    const permutations = [];
...
function makePossibleGrids(startCells, startGrid) {
    const grids = [];
    const permutations = makeNeighborsPermutations();
    function traverse(grid, gridSet, cellIter, valueIter) {
        const startNumbers = [];
        ...
        for (let start of startNumbers) {
            const neighbors = helpers_1.get4Neighbors(grid, start);
            for (let n of neighbors) {
            ...
            for (let p of permutations) {
                ...
                for (let [i, n] of neighbors.entries()) {
                    if (n !== null && newGrid[n.x][n.y] === 0) {
                        if (p[i]) {
                            const nextNeighbors = helpers_1.get4Neighbors(newGrid, n).filter(n => n);
                            if (nextNeighbors.every((n) => ((newGrid[n.x][n.y] === 0 || newGrid[n.x][n.y] === id)))) {
                                newGrid[n.x][n.y] = id;
                                nextCells++;
                            }
                        }
...
//# sourceMappingURL=nurikabeSolver.js.map